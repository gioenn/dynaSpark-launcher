...
class Config(object):
    ...
    GIT_XSPARK_REPO = "https://github.com/gioenn/xSpark.git" # "https://github.com/DavideB/xSpark.git" # "https://github.com/gioenn/xSpark.git"
    GIT_XSPARK_BRANCH = "symb" # " was symb symex_0.6" #"symex_0.5"   #  "exp_2018"     # "xSpark-1.0"
    ...
    cfg_dict = {    "AzKeyName": AZ_KEY_NAME,
                    ...
                    "RamDriverMaxResultSize": RAM_DRIVER_MAX_RESULT_SIZE,
                    "RamExec": RAM_EXEC,
                    "OffHeap": OFF_HEAP,
                    ...
                    "GitXsparkRepo": GIT_XSPARK_REPO,
                    "GitXsparkBranch": GIT_XSPARK_BRANCH,
                    "GitXsparkRepo": GIT_XSPARK_DAGSYMB_REPO,
                    "GitXsparkBranch": GIT_XSPARK_DAGSYMB_BRANCH
                    }
    ...
    def config_experiment(self, filepath, cfg):
        ...
        if exp_file.exists():
            ...
            if len(experiment) > 0:
                ...
                if "AppConf" in keys and "AppName" in keys:
                    cfg["appconf"] = {}
                    application = experiment["AppName"]
                    cfg["experiment"]["app_name"] = application
                    self.cfg_dict["AppConf"][application] = self.APP_CONF[application] = {}
                    for k_app_conf in experiment["AppConf"].keys():
                        self.cfg_dict["AppConf"][application][k_app_conf] = self.APP_CONF[application][k_app_conf] = cfg["appconf"][k_app_conf] = str(experiment["AppConf"][k_app_conf])
    ...
    def config_app(self, app, cfg):
        
        num_partitions = cfg['main']['num_partitions']
        print('setting NUM_OF_PARTITIONS as {}'.format(num_partitions))                                                                      
        self.update_config_parms(self)
    ...
    def config_setup(self, filepath):
        ...
        if setup_file.exists():
            ...
            print("Setup file:  " + filepath + " not found: using defaults")
