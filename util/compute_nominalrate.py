...
def mean_nominalrate(fold):
    ...
    with open(fold + "/result.txt", "w") as out:
        for id in sorted(stageDict.keys()):
            out.write(
                "SID " + str(id) + " MEAN " + str(np.mean(stageDict[id]["nominalrate"])) + "\n")
            ...
            out.write("\n")
            out.write("SID " + str(id) + " % " + str((
                                                     np.std(stageDict[id]["nominalrate"]) / np.mean(
                                                         stageDict[id][
                                                             "nominalrate"])) * 100) + "\n")
            out.write("\n")
...
output_folder = "./results/OK/PageRank/Profiling-NominalRate/"
# output_folder = "./spark-bench/num/app-20161013104553-0000"
import os
...
plt.ylabel('% std dev')
plt.xlabel("Stage ID")
plt.savefig(output_folder + "result.png")
plt.close()
...
plt.ylabel('MIN DIFF')
plt.xlabel("Stage ID")
plt.savefig(output_folder + "min.png")
plt.close()
