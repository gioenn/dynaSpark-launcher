...
def plot(folder):
    ...
    :return:
    """               
    print("Generating charts from data in folder:", folder)
    # comment the following line to append the chart to the charts generated by the same run 
    charts['Charts'] = []    # comment this line to append the chart to the charts generated by the same run
    ...
    print("app_logs: ", app_logs)
    app_info = {}
    for app_log in sorted(app_logs):
        app_info = load_app_data(app_log)

        for app_id in app_info:
            if app_id == max(app_info.keys()):  # DB- Dagsymb: consider only the latest app_id in log (i.e. discard all initial data-generation app_id's
