...
def gather_records_rw(stages):
    not_skipped = {k: v for k, v in stages.items() if k != "jobs" and v['skipped'] == False}
...
def main(input_dir=INPUT_DIR, json_out_dir=OUTPUT_DIR, reprocess=False):
    ...
    for log in glob.glob(os.path.join(input_dir, 'app-*')):
        ...
        app_act_end_time = 0
        dat_folder = 'home/ubuntu/dagsymb/num/'+ log.split('.')[0].split(os.sep)[-1]
        ...
        try:
            with utils.open_cfg(rpath=dat_folder) as cfg:
                profile_suffix = cfg['experiment']['profile_name'].split("-")[-1]
        except Exception:
